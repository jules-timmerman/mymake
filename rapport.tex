...
\begin{algorithm}
\caption{\texttt{makefile2list}}
\label{alg:cap}
\begin{algorithmic}
\State L = \texttt{createListeRegles}
\State R = \texttt{createRegle("", [], 0, [])}
\State enregistré = \texttt{VRAI}
\State ligne = première ligne de \textbf{makefile}
\While{il reste des lignes à lire}
\If{ligne est vide et enregistré = \texttt{FAUX}}
	\State enregistré $\gets$ VRAI
	\State R.commandes $\gets$ \texttt{revListCommande}(R.commandes)
	\State L $\gets$ \texttt{addRegle(L,R)}
	\State R $\gets$ \texttt{createRegle("", [], 0, [])}
\ElsIf{ligne est de type \texttt{nom : <listePrerequis>}}
	\State R.nom $\gets$ \texttt{nom}
	\State R.prerequis $\gets$ \texttt{<listePrerequis>}
	\State R.lenPrerequis $\gets$ |\texttt{<listePrerequis>}|
\Else \Comment{ligne de type \texttt{<commande>}}
    \State R.commandes $\gets$ \texttt{ligne::}R.commandes
%\ElsIf{$N$ is odd}
%    \State $y \gets y \times X$
%    \State $N \gets N - 1$
\EndIf
\EndWhile
\If{enregistré = \texttt{FAUX}}
	\State R.commandes $\gets$ \texttt{revListCommande}(R.commandes)
	\State L $\gets$ \texttt{addRegle(L,R);}
\EndIf
