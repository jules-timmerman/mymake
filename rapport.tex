...
\begin{itemize}
	...
	Elle prends en argument la liste sur laquelle on souhaite appliquer la fonction, la liste générale de règles qui sera passée en argument de la fonction, un pointeur vers la fonction à appliquer et un booléen permettant de décider comment gérer les potentielles règles \texttt{NULL}.
...
gcc -o main main.o a.o b.o c.o d.o
\end{verbatim}
et crée les fichiers binaires. Réitérer la commande ne fait rien. Modifier \textbf{c.h} et relancer la commande affiche
\begin{verbatim}
gcc -c c.c
\end{verbatim}
et crée un nouveau fichier \textbf{c}. Le programme se comporte comme attendu.

On obtient le même succès lorsque l'on tente \texttt{./mymake main} dans le dossier des fichiers source. On a donc décidé de l'ordre des compilations de \texttt{myMake} avec lui-même !
...
\begin{question} %% Question 10 = extensions
	...
	une implémentation en SHA-256 en C\cite{hash}. La librairie est principalement composée de trois fonctions et une structure.
	\begin{itemize}
